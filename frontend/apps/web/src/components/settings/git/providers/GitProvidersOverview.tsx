    ...
    return (
        <>
            ...
            <div className='container mx-auto py-8'>
                ...
                <div className='mt-8'>
                    <h2 className='text-xl font-semibold mb-4'>Connected GitHub Providers</h2>
                                    ...
                                    <li key={provider.id} className='p-4 flex items-center justify-between'>
                                        <div className='flex items-center space-x-3'>
                                            ...
                                            <div className='flex flex-col'>
                                                <span className='font-medium dark:text-gray-200'>{provider.name}</span>
                                                <span className='text-sm text-gray-500 dark:text-gray-400'>
                                                    Added {formatDate(provider.createdAt, 'MMMM d, yyyy HH:mm')}
                                                </span>
                                            </div>
                                        </div>
                                        <span className={`text-sm ${provider.isConnected === true ? 'text-green-500 dark:text-green-400' : provider.isConnected === false ? 'text-red-500 dark:text-red-400' : 'text-gray-500 dark:text-gray-400'}`}>
                                            {provider.isConnected === true ? 'Connected' : provider.isConnected === false ? 'Not connected' : 'Unknown'}
                                        </span>
                                    </li>
                                ))}
                            </ul>
