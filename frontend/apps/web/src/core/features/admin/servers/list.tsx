import { GetServersQueryResponse } from '@/src/core/generated'
import { Card } from '@ui/components/ui/card.tsx'
import { Button } from '@ui/components/ui/button.tsx'
import { CircleIcon, GaugeIcon, HardDriveIcon, MemoryStickIcon, ServerIcon } from 'lucide-react'
import Link from 'next/link'


const StateMap = {
    Offline: <div className="flex items-center gap-2 text-sm text-muted-foreground">
        <CircleIcon className="w-2 h-2 fill-red-500"/>
        <span>Offline</span>
    </div>,
    Maintenance: <div className="flex items-center gap-2 text-sm text-muted-foreground">
        <CircleIcon className="w-2 h-2 fill-yellow-500"/>
        <span>Maintenance</span>
    </div>,
    Online: <div className="flex items-center gap-2 text-sm text-muted-foreground">
        <CircleIcon className="w-2 h-2 fill-green-500"/>
        <span>Online</span>
    </div>
}

export const ServersList = ({ servers }: { servers: GetServersQueryResponse }) => {
