...
public class DockerComposeUpBuildStepGenerator(DockerComposeUpBuildStepGenerator.Args args)
    : IBuildStepGenerator
{
    public DeploymentBuildStep Generate()
    {
        return new DeploymentBuildStep
        {
            ...
            Description =
                "Runs docker compose up and stores the service names in a context, if enabled.",
            Type = DeploymentBuildStepType.DockerComposeUp,
            Params = new Dictionary<string, object>
            {
                { "dockerComposeFile", args.DockerComposeFile },
                { "storeServiceNames", args.StoreServiceNames },
            },
        };
    }
