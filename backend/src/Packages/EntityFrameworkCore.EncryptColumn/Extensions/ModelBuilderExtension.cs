...
public static class ModelBuilderExtension
{
    public static void UseEncryption(this ModelBuilder modelBuilder, IEncryptionProvider encryptionProvider)
    {
        ...
        foreach (IMutableEntityType entityType in modelBuilder.Model.GetEntityTypes())
        {
            ...
            {
                if (property.ClrType == typeof(string) && !IsDiscriminator(property) && property.PropertyInfo != null)
                {
                    object[] attributes =
                        property.PropertyInfo.GetCustomAttributes(typeof(EncryptColumnAttribute), false);
                    if (attributes.Any())
