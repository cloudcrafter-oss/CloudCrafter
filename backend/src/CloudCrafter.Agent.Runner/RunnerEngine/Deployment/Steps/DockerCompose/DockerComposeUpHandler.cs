    ...
    : IDeploymentStepHandler<DockerComposeUpParams>
{
    ...
    public async Task ExecuteAsync(DockerComposeUpParams parameters, DeploymentContext context)
    {
        ...
        if (context.Recipe.DockerComposeOptions == null || string.IsNullOrWhiteSpace(context.Recipe.DockerComposeOptions.DockerComposeDirectory))
        {
            throw new DeploymentException("Docker compose options not found - cannot write docker compose file.");
        }
        
        var dockerComposeFile = $"{context.Recipe.DockerComposeOptions.DockerComposeDirectory}/{parameters.DockerComposeFile}";
