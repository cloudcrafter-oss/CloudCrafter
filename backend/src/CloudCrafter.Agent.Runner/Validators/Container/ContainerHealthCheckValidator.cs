...
public class ContainerHealthCheckValidator : AbstractValidator<ContainerHealthCheckParams>
{
    public ContainerHealthCheckValidator()
    {
        RuleFor(x => x.Services)
            .ForEach(service =>
                service.SetValidator(new ContainerHealthCheckParamsServiceValidator())
            );
    }
}

public class ContainerHealthCheckParamsServiceValidator
    : AbstractValidator<KeyValuePair<string, ContainerHealthCheckParamsOptions>>
{
    public ContainerHealthCheckParamsServiceValidator()
    {
        RuleFor(x => x.Key).NotEmpty().WithMessage("Service name is required.");
...
public class ContainerHealthCheckParamsOptionsValidator
    : AbstractValidator<ContainerHealthCheckParamsOptions>
{
    public ContainerHealthCheckParamsOptionsValidator()
    {
        RuleFor(x => x.CheckForDockerHealth)
            .NotNull()
            .When(x => !HasHttpChecks(x))
        ...
        RuleFor(x => x.BackOffPeriod)
            .GreaterThan(0)
            .When(x => x.BackOffPeriod.HasValue)
