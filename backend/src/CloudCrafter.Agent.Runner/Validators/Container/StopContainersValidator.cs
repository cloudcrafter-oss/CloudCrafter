...
public class StopContainersValidator : AbstractValidator<StopContainersParams>
{
    ...
    public StopContainersValidator()
    {
        RuleFor(x => x.Filters)
            ...
            .WithMessage($"Filters must only contain the following keys: {string.Join(", ", AllowedFilterKeys)}");
        
        
        When(x => x.Filters.ContainsKey("labels"), () =>
        {
            RuleFor(x => x.Filters["labels"])
                ...
                .ForEach(labelRule =>
                {
                    labelRule.Must(label => DockerLabelFilter.TryParse(label, out _))
                        .WithMessage((_, label) => $"'{label}' is not a valid Docker label filter. It should be in the format 'key=value' or 'key!=value'.");
                });
        });
    }
}
