...
public class DockerComposeHelper(
    ...
    ILogger<DockerComposeHelper> logger
) : IDockerComposeHelper
{
    public async Task<ExecutorResult> UpAsync(
    ...
    public async Task<string> GetDockerContainerIdForService(
        string dockerComposeFile,
        string serviceName
    )
    {
    ...
    public async Task<Dictionary<string, string>> GetDockerComposeServices(string dockerComposeFile)
    {
        try
        {
            var result = await executor.ExecuteAsync(
                "docker",
                ["compose", "-f", dockerComposeFile, "ps", "--format", "json"]
            );

            if (string.IsNullOrWhiteSpace(result.StdOut))
            {
                throw new AgentDockerException(
                    "No output for given command, file: " + dockerComposeFile
                );
            }
            ...
            {
                var container = JsonSerializer.Deserialize<DockerComposeDockerComposePsOutput>(
                    serviceAsJson,
                    options
                );
                ...
                {
                    throw new AgentDockerException(
                        "Could not deserialize docker compose ps output, file: " + dockerComposeFile
                    );
                }
        ...
        {
            throw new AgentDockerException(
                $"Failed to get docker container names for services, file: {dockerComposeFile}",
                ex
            );
        }
    }
