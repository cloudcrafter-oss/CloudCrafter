    ...
    >
{
    ...
    public void Setup()
    {
        ...
        _mockPump
            .Setup(p => p.CreateLogger<DockerComposeWriteToFileSystemHandler>())
            .Returns(_mockLogger.Object);

        _handler = new DockerComposeWriteToFileSystemHandler(
            _mockPump.Object,
    ...
    public async Task ExecuteAsync_ShouldWriteDockerComposeFile()
    {
        ...
        var base64DockerCompose = Convert.ToBase64String(
            Encoding.UTF8.GetBytes(dockerComposeContent)
        );
        ...
        // Assert
        _mockLogger.Verify(
            l => l.LogInfo("Writing docker compose file to the file system"),
            Times.Once
        );
        _mockLogger.Verify(
            l => l.LogInfo($"Successfully wrote docker compose file to {expectedFullPath}"),
            Times.Once
        );
        ...
        _mockFileSystemHelper.Verify(
            f => f.WriteFile(expectedFullPath, dockerComposeContent),
            Times.Once
        );
    }
    ...
    public void ExecuteAsync_ShouldThrowDeploymentException_WhenBase64DockerComposeIsEmpty()
    {
        ...
        _context.Recipe.DockerComposeOptions = new()
        {
            Base64DockerCompose = string.Empty,
            DockerComposeDirectory = "/app/docker",
        };
    ...
    public void ExecuteAsync_ShouldThrowDeploymentException_WhenDockerComposeDirectoryIsEmpty()
    {
        ...
        _context.Recipe.DockerComposeOptions = new()
        {
            Base64DockerCompose = "c29tZSBjb250ZW50",
